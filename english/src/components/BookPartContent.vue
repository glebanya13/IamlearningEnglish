  ...
  <v-card class="pa-2">
      ...
      <div class="mt-2">
          ...
          <v-tabs v-model="tabMode" fixed-tabs>
              ...
              <v-tab :key="'sidebyside'">
                  Параллельно
              </v-tab>
              <v-tab-item :key="'english'">
                  <div v-for="(paragraph, i) in part.content" :key="`par1${i}`">
                      <span>&nbsp;&nbsp;</span>
                      <span v-for="(sentence, y) in paragraph.sentences" :key="`par1${i}sen1${y}_orig`" :style="textStyle">
                          <span>
                              {{sentence.origText}}
                          </span>
                          <v-icon @click.prevent="toggleVisibility(i, y)" :size="fontSize">help</v-icon>
                          <span v-if="getVisibilityFlag(i, y).value" class="primary--text" style="font-weight:bold">
                              {{sentence.transText}}
                          </span>
                      </span>
                  </div>
